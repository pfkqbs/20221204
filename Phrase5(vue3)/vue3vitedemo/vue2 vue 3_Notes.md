...
- 4 组件中修改 vuex 中的数据： `$store.dispatch('action中的方法名'，数据)` 或者 `$store.commit('mutations中的方法名'，数据)`
> 备注： 若没有网络请求或其他业务逻辑，组件中也可以越过`actions`，即不写`dispatch` ,直接写 `commit`

- 5 `getters` 的使用
	- 概念： 当`state` 中的数据需要经过加工后再使用时，可以使用`getters` 加工
	- 再`store.js` 中追加 `getters` 配置
	```js
	//...
	const getters = {
		bigsum(state){
...
- 6 四个 `map` 方法的使用：
	...
	```js
	computed:{
		//借助mapActions 生成计算属性： bigSum ( 对象写法 )
		...mapActions({bigSum:"bigSum"}),

		//借助mapActions 生成计算属性： bigSum ( 数组写法 )
		...mapActions(["bigSum"]),
	}
	```
	...
	```js
	computed:{
		//借助 mapMutations 生成计算属性： bigSum ( 对象写法 )
		...mapMutations({increment:"JIA",decrement:"JIAN"}),

		//借助 mapMutations 生成计算属性： bigSum ( 数组写法 )
		...mapMutations(["bigSum"]),
	}
	```
